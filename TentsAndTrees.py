...
class Board:
    ...
    def switch_type(self, x, y):
        ...
        >>> isinstance(b.object_at(0, 0), EmptyTile)
        False
        >>> isinstance(b.object_at(0 ,0), Grass)
        True
        """
...
class Tent (TileObject):
    ...
    def valid_position(self) -> bool:
        """
        Check all sides of the tent to see if there is another tent around. if
...
class Grass (TileObject):
    ...
    def swap_type(self):
        Tent(self.get_x(), self.get_y(), self._board)
...
class EmptyTile (TileObject):
    ...
    def swap_type(self):
        Grass(self.get_x(), self.get_y(), self._board)
...
class Game:
    ...
    def place_tent(self, ran_x, ran_y) -> bool:
        ...
        if isinstance(object_at_position, EmptyTile):
            ...
            new_tent = Tent(ran_x, ran_y, self.get_board())
            for direction in CARDINAL_DIRECTIONS:
                ...
                if not (new_tree_y < 0) and not (new_tree_y >= self.get_board().get_height()):
                    if not (new_tree_x < 0) and not (new_tree_x >= self.get_board().get_width()):
                        ...
                        if isinstance(tree_space, EmptyTile):
                            new_tree = Tree(new_tree_x, new_tree_y, self.get_board())
    ...
    def get_invalid_positions(self) -> List[TileObject]:
        ...
        >>> g = Game()
        >>> g.new_board(5, 5)
        >>> e1 = Tent(0,0, g.get_board())
        >>> len(g.get_invalid_positions())
        4
        >>> e1 = Tent(1,0, g.get_board())
        >>> len(g.get_invalid_positions())
        6
        """
